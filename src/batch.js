...
let debug = require('debug')('perjury:batch');

import atMostOnce from './at-most-once';
...
class Batch {
  ...
  constructor(title, definition) {
    ...
    this.title = title;
    assert(_.isString(this.title), "Batch title must be a string");
    ...
    for (let name in definition) {
      ...
      if (name === "topic") {
        ...
        assert(_.isFunction(value), `'topic' of '${this.title}' must be a function`);
        this.topic = value;
    ...
    assert(_.isFunction(this.topic), `Batch '${this.title}' has no topic`);
