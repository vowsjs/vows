...
// Object
//
assert.isEnumerable = function (obj, key, message) {
    ...
    if (typeof(obj[key]) == "undefined") {
        assert.fail(null, null, "key '" + key + "' was never defined", "isEnumerable", assert.isEnumerable);
    }
...
function assertAnyTypeOf(actual, expected, message, caller) {
    if (expected.indexOf(typeOf(actual)) == -1) {
        assert.fail(actual, expected, message || "expected {actual} to be any type of {expected}", "typeOf", caller);
    }
}
