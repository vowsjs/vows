    ...
    if (batch.honored + batch.broken + batch.errored + batch.pending === batch.total &&
        ...
        if (batch.teardowns) {
            return async.forEachSeries(
                batch.teardowns.reverse(),
                function runTeardown(teardown, next) {
                    ...
                    // This should be configurable, but for right now
                    // it will suffice.
                ...
                function tryFinish() {
                    ...
                    if (pending.length > 0) {
                        // TODO (indexzero): Log something about teardowns which
                        // did not complete in 2000ms
                    }
