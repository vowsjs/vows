...
this.Suite.prototype = new(function () {
    ...
    this.parseBatch = function (batch, matcher) {
        ...
        (function count(tests, _match) {
            ...
            match = keys.some( function (key) {
              return !tests[key]._skip;
            });
...
//            }
    ...
    this.runBatch = function (batch) {
        ...
        (function run(ctx, lastTopic) {
            ...
            var subkeys = Object.keys(ctx.tests).filter(function (k) {
                return ctx.tests[k] && k !== 'topic'    &&
                                       k !== 'teardown' && !ctx.tests[k]._skip;
            });
            topic.vowsCount = subkeys.length;
            console.log("vowsCountZERO " + topic.vowsCount);
            ...
            // Teardown
            topic.addListener("teardown", function () {
                console.log("TEARDOWN " + ctx.name);
                if (ctx.tests.teardown) {
                    ctx.tests.teardown.apply(ctx.env, ctx.topics);
                }
                if (ctx.parentCtx.emitter && ctx.parentCtx.emitter.vowsCount) {
                    if (--ctx.parentCtx.emitter.vowsCount === 0) {
                        ctx.parentCtx.emitter.emit("teardown");
                    }
                    console.log(ctx.parentCtx.emitter.vowsCount);
                }
            });
            if (! ctx.tests._skip) {
                batch.remaining --;
