...
function addVow(vow) {
    ...
    this.on(event, function () {
        ...
        var args = vow.binding.context.nextTopics().slice();
        if (!(this.ctx && this.ctx.isEvent) && batch.suite.options.error) {
            // Temporarily compensate for a better solution to overall
            // error handling
            if (args.length == 1 && !(args[0] instanceof Error) &&
                vow.callback.length == 2) {
                args.unshift(null);
            }
        }
