...
/*
 * path, events, util are all core node modules.
 *
 * util is primarily to support node's internal modules
 * path provides methods for working with file and directory paths
 * events is the event emitter api for working with node events
 */
var path = require('path'),
    events = require('events'),
    util = require('util'),
    // node maintains the exports object as a pointer to module.exports
    // would it be better to update this to module.exports?
    // here we are renaming the exports object to our module, so that we can attatch methods to it
    vows = exports;
...
/*
 * Define a getter which will respond to a reporter event (?)
 */
vows.__defineGetter__('reporter', function () {
    return vows.options.reporter;
});
// defines the output of vows tests
var stylize = require('./vows/console').stylize;
// reasseign the global console object, this double assignment looks like a bad code practice
// this lets us use the console object to output test results
...
// what does eventEmitter.on give us?

// goodness.
//
function addVow(vow) { // vow is an object
  // console.log(vow)
    var batch = vow.batch, // also an object
        event = vow.binding.context.event || 'success',

        self = this; // why are we using 'this' here?

    batch.total ++; // increment batch
    batch.vows.push(vow); //
    ...
    function output(status, exception) {
        ...
        // where is the report function defined?
    ...
    vows.suites.forEach(function (s) {
        ...
        s.batches.forEach(function (b) {
            ...
            if (unFired.length > 0 || expected.length > 0) {
              // why use the global object here over console?
                global.console.log('\n');
            }
