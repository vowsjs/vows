...
var sys = require('sys'),
    assert = require('assert'),
    inspect = require('eyes').inspector({writer: function () {}}),
    vows = exports;
...
vows.macros = {
    ...
    match: function (actual, expected) { assert.ok(actual.match(expected)) },
    include: function (actual, item) { assert.ok(actual.match(new(RegExp)(item))) },
    beA: function (actual, type) {
        if (typeof(actual) === 'object') { assert.ok(actual.constructor === type) }
        else {
