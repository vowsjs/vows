...
class Batch {
  constructor (title, definition) {
    ...
    for (const name in definition) {
      ...
      if (name === 'topic') {
        ...
        debug(`Adding error handler to ${this.title}`)
        assert.isFunction(value,
          `'error' of ${this.title} must be a function`)
        this.error = value
  ...
  runTests (err, results) {
    ...
    if (_.size(this.tests) > 0) {
      const args = (this.error) ? results : _.concat([err], results)
