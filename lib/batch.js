...
class Batch {
  ...
  run(args, callback) {
    ...
    try {
      results = this.topic.apply({callback: next}, args);
    } catch(err) {
  ...
  onTopicComplete(args, callback) {
    ...
    return atMostOnce((function() {
      ...
      successes = _.keys(this.tests).length - failures;
      ...
      assert.equal(successes + failures, _.keys(this.tests).length, `failures + successes != number of tests`);
